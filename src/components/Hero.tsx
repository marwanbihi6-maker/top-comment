        ...
        <div className="flex flex-col lg:flex-row items-center gap-6 lg:gap-20">
          ...
          <div className={`flex-1 space-y-5 lg:space-y-8 max-w-2xl transition-all duration-1000 transform ${mounted ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-center pt-2 w-full sm:w-auto">
              <Button onClick={scrollToShop} size="md" className="group w-full sm:w-auto py-3 text-sm md:px-8 md:py-4 md:text-lg">
                Shop Now 
                <ArrowRight className="ml-2 w-4 h-4 sm:w-5 sm:h-5 group-hover:translate-x-1 transition-transform" />
              </Button>
              <Button variant="outline" size="md" className="w-full sm:w-auto py-3 text-sm md:px-8 md:py-4 md:text-lg">
                View Lookbook
              </Button>
            </div>
          ...
          <div className={`flex-1 relative w-full min-h-[320px] sm:min-h-[500px] lg:h-auto flex items-center justify-center lg:justify-end transition-all duration-1000 delay-300 transform ${mounted ? 'translate-x-0 opacity-100' : 'translate-x-10 opacity-0'} mt-4 lg:mt-0`}>
            ...
            <div className="relative w-full max-w-[340px] sm:max-w-[500px] lg:max-w-[650px] aspect-square flex items-center justify-center">
              ...
              {/* 3. Hoodie (Back Right - Deepest Layer) */}
              {/* Positioned to the right and back, creating depth */}
              <div className="absolute top-1/2 left-1/2 w-[48%] lg:w-[46%] transition-all duration-500 ease-out z-10
                              -translate-y-[60%] translate-x-[15%] rotate-6 opacity-80 
                              hover:z-40 hover:scale-105 hover:opacity-100 hover:rotate-0">
                <img 
                  src="https://i.postimg.cc/X7hMG0Fh/here-we-are-2026-(1).png" 
                  alt="Streetwear Hoodie Collection" 
                  className="w-full h-auto rounded-2xl sm:rounded-3xl shadow-xl border border-white/5"
                />
              </div>

              {/* 2. Tee 2 (Back Left - Middle Layer) */}
              {/* Positioned to the left and slightly forward of the hoodie */}
              <div className="absolute top-1/2 left-1/2 w-[45%] lg:w-[44%] transition-all duration-500 ease-out z-20
                              -translate-y-[50%] -translate-x-[115%] -rotate-6 opacity-90
                              hover:z-40 hover:scale-105 hover:opacity-100 hover:rotate-0">
                <img 
                  src="https://i.postimg.cc/0ywLWLY3/here-we-are-2026-(2).png" 
                  alt="Streetwear Tee Design" 
                  className="w-full h-auto rounded-2xl sm:rounded-3xl shadow-xl border border-white/5"
                />
              </div>

              {/* 1. Main Tee (Front Center - Top Layer) */}
              {/* The Hero Product - Centered, Largest, Highest Contrast */}
              <div className="absolute top-1/2 left-1/2 w-[60%] lg:w-[55%] transition-all duration-500 ease-out z-30
                ...
                <img 
                  ...
                  className="w-full h-auto rounded-2xl sm:rounded-3xl shadow-[0_25px_50px_-12px_rgba(0,0,0,0.7)] border border-white/10"
                />
              </div>
